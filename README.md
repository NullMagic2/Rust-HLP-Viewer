...
- Repairs the two stale Windows GDI font calls left by build-fix 85. Printer text and on-screen retained text now both call the centralized `create_gdi_font_for_style(style, dpi_y, zoom)` helper directly, restoring Windows compilation after the duplicate wrapper methods were removed.
- Removes production-unused public convenience functions from `HelpDocument`, `NavigationHistory`, `LayoutEngine`, and `FontDescriptor`: direct topic-store exposure, first-topic shortcuts, boolean history probes, deterministic layout wrappers, the legacy generic metric converter, and the unused RTL charset predicate.
- Preserves deterministic layout coverage through a private test-only helper that drives `layout_topic_with_measurer`; history and charset tests now assert the underlying state directly instead of keeping otherwise unused production methods alive.
- Leaves the supported viewer path and parser behaviour unchanged while reducing the public API to functions exercised by production code or intentional external-facing functionality.

## v0.7.1-buildfix85

- Removes the unreachable detached auxiliary-window implementation left behind after build-fix 16 standardized all topic destinations on the single main help surface. This deletes the obsolete auxiliary frame/state types, popup sizing and positioning, duplicate auxiliary painting, hotspot/tooltip handlers, macro execution, linked-document installation, bookmarks, lifetime handling, and the inert `active_popup` state.
- Removes the superseded retained-box HTML renderer. HTML export continues to use the semantic document-flow implementation introduced by build-fix 77.
- Consolidates the main jump/popup macro paths into one main-surface navigation function and removes obsolete window-routing helpers and popup-anchor plumbing. `SetPopupColor` remains parsed safely but reports that detached popup windows are disabled.
- Centralizes WinHelp compressed integers and signed-count validation in the shared binary reader; centralizes path identity, automatic relative-help reference policy, and symbol-face detection in viewer support; and shares border-clearance, small-caps point-size, and compact table-cell filler semantics between retained layout and HTML export.
- Removes the duplicate non-selection painter, duplicate GDI font factory wrappers, a production-unused font metadata helper, and an unused `StringStream::next_string` wrapper.
- Net result: 1,836 fewer Rust source lines than build-fix 84, including 1,538 lines removed from `viewer/src/main.rs` and 310 from `viewer/src/html_export.rs`, without intentionally removing any currently reachable viewer feature.
