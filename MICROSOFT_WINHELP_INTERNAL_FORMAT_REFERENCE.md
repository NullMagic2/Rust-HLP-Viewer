Build-fix 85 is a conservative dead-code and duplication cleanup. It removes the unreachable detached auxiliary-window subsystem and the superseded retained-box HTML renderer, then centralizes shared binary-decoding, path-policy, font-semantic, border, and table-filler helpers. Current single-surface navigation, semantic HTML export, printing, images, tooltips, selection, search, and retained layout remain the supported paths. The cleanup reduces the Rust source tree by 1,836 lines relative to build-fix 84.
