...
impl FontDescriptor {
    ...
    /// Reports the two charsets that enter WinHlp32's special RTL run-reordering path.
    pub const fn is_rtl_charset(&self) -> bool {
        matches!(self.charset, Some(0xB1 | 0xB2))
    }
}
...
impl FontTable {
    ...
    pub(crate) fn apply_face_charsets(&mut self, charsets: &[u8]) {
        for descriptor in &mut self.descriptors {
            descriptor.charset = charsets.get(descriptor.face_index).copied();
        }
    }
...
mod tests {
    ...
    fn face_charset_table_is_applied_by_face_index() {
        ...
        table.apply_face_charsets(&[0xB1]);
        assert_eq!(table.descriptors()[0].charset, Some(0xB1));
        assert!(table.descriptors()[0].is_rtl_charset());
    }
