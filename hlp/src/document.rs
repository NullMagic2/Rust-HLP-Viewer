...
impl HelpDocument {
    ...
    /// Returns the decoded topic store.
    pub const fn topic_store(&self) -> &TopicStore {
        &self.topics
    }
    ...
    /// Returns the first reconstructed topic, when one exists.
    pub fn first_topic(&self) -> Option<&Topic> {
        self.topics.topics().first()
    }

    /// Returns the first presentation topic, when one exists.
    pub fn first_presentation(&self) -> Option<&TopicPresentation> {
