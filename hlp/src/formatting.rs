...
impl FormattedRecord {
    ...
    fn decode_with_font_context(
        ...
        let topic_size = read_compressed_signed_long(&mut ld1)?;
        let topic_length = if is_modern_record(record.record_type) {
            Some(read_compressed_unsigned_short(&mut ld1)?)
        } else {
            None
        };
...
/// Reads WinHelp's one/two-byte compressed unsigned short.
...
/// Reads WinHelp's one/two-byte compressed signed short.
fn read_compressed_signed_short(reader: &mut Reader<'_>) -> Result<i16, HlpError> {
    let first = reader.read_u8()?;
    let value = if first & 1 == 0 {
        i32::from(first) / 2 - 64
    } else {
        let second = reader.read_u8()?;
        i32::from(first) / 2 + i32::from(second) * 128 - 16_384
    };
    i16::try_from(value).map_err(|_| HlpError::invalid("compressed short", "decoded value exceeds i16"))
}

/// Reads WinHelp's two/four-byte compressed unsigned long.
#[cfg(test)]
...
/// Reads WinHelp's two/four-byte compressed signed long.
fn read_compressed_signed_long(reader: &mut Reader<'_>) -> Result<i32, HlpError> {
    let first = reader.read_u16()?;
    let value = if first & 1 == 0 {
        i64::from(first) / 2 - 16_384
    } else {
        let second = reader.read_u16()?;
        // KB917607 WinHlp32 0x4129e8 reads the complete little-endian DWORD,
        // shifts it right once, then subtracts 0x40000000. Expressing the DWORD
        // as two words gives first/2 + second*32768 - 1_073_741_824.
        i64::from(first) / 2 + i64::from(second) * 32_768 - 1_073_741_824
    };
    i32::try_from(value).map_err(|_| HlpError::invalid("compressed long", "decoded value exceeds i32"))
}
...
fn read_compact_record_header(reader: &mut Reader<'_>) -> Result<CompactRecordHeader, HlpError> {
    ...
    let (payload_size, topic_length) = if matches!(raw_type, 0x02 | 0x21) {
        ...
        let size = read_compressed_signed_long(reader)?;
        if size < 0 {
            return Err(HlpError::invalid(
                "compact record",
        ...
        let topic_length = if raw_type > 0x10 {
            Some(read_compressed_unsigned_short(reader)?)
        } else {
            None
        };
        (
...
fn parse_paragraph_info(reader: &mut Reader<'_>) -> Result<ParagraphFormat, HlpError> {
    ...
    let _paragraph_leading_value = read_compressed_signed_long(reader)?;
    let id = reader.read_u16()?;
    let bits = reader.read_u16()?;

    let unknown_value = conditional(bits, 0, || read_compressed_signed_long(reader))?;
    let spacing_above = conditional(bits, 1, || read_compressed_signed_short(reader))?;
    let spacing_below = conditional(bits, 2, || read_compressed_signed_short(reader))?;
    let spacing_lines = conditional(bits, 3, || read_compressed_signed_short(reader))?;
    let left_indent = conditional(bits, 4, || read_compressed_signed_short(reader))?;
    let right_indent = conditional(bits, 5, || read_compressed_signed_short(reader))?;
    let first_line_indent = conditional(bits, 6, || read_compressed_signed_short(reader))?;
    ...
    let default_tab_interval = conditional(bits, 7, || read_compressed_signed_short(reader))?;
...
fn parse_tabs(reader: &mut Reader<'_>) -> Result<Vec<TabStop>, HlpError> {
    let count = read_compressed_signed_short(reader)?;
    if count < 0 {
    ...
    for _ in 0..count {
        let raw = read_compressed_unsigned_short(reader)?;
        let alignment = if raw & 0x4000 != 0 {
            match read_compressed_unsigned_short(reader)? {
                0 => TabAlignment::Left,
...
impl<'a> StringStream<'a> {
    ...
    fn next_string(&mut self) -> Result<Option<String>, HlpError> {
        self.next_string_for_font(0)
    }
...
mod tests {
    ...
    fn compressed_numbers_follow_winhelp_bias_rules() {
        ...
        assert_eq!(read_compressed_unsigned_short(&mut reader).unwrap(), 10);
        assert_eq!(read_compressed_unsigned_short(&mut reader).unwrap(), 258);
        assert_eq!(read_compressed_unsigned_long(&mut reader).unwrap(), 2);
        assert_eq!(read_compressed_unsigned_long(&mut reader).unwrap(), 32_770);
        ...
        assert_eq!(read_compressed_signed_long(&mut signed).unwrap(), 0);
        assert_eq!(read_compressed_signed_long(&mut signed).unwrap(), 0);
